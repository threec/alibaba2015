...
def GenFeature(finput='user_action_train.csv', foutput = 'feature.csv', lastday = '2014-12-18'):
		...
		for row in reader:
			...
			if row[2] == '1': # click
				if diff_time < 3*24*3600: # last three day
					...
					IncDict(user_item_lastThreeDay_click, ucid)
				elif diff_time < 6*24*3600: # last six day
					IncDict(user_item_lastSixDay_click, ucid)
			...
			if row[2] == '2': # star
				...
				if diff_time < 3*24*3600: # last three day
					IncDict(user_item_lastThreeDay_star, ucid)
				elif diff_time < 6*24*3600: # last six day
					IncDict(user_item_lastSixDay_star, ucid)
			...
			if row[2] == '3': # add to car
				...
				if diff_time < 3*24*3600: # last three day
					IncDict(user_item_lastThreeDay_add_car, ucid)
				elif diff_time < 6*24*3600: # last six day
					IncDict(user_item_lastSixDay_add_car, ucid)
				else:  # before half month
					pass
			...
			if row[2] == '4':  # buy
				...
				# print utid
				if diff_time < 3*24*3600: # last three day
					IncDict(user_item_lastThreeDay_buy, ucid)
			
				elif diff_time < 6*24*3600: # last six day
					IncDict(user_item_lastSixDay_buy, ucid)
				else:  # before half month
					pass
	...
	for key in user_items:
		...
		data = [uid, tid,
			GetDict(user_item_lastSixDay_click, ucid)-GetDict(user_item_lastThreeDay_click, ucid),
			GetDict(user_item_lastSixDay_star, ucid)-GetDict(user_item_lastThreeDay_star, ucid),
			GetDict(user_item_lastSixDay_add_car, ucid)-GetDict(user_item_lastThreeDay_add_car, ucid),
			GetDict(user_item_lastSixDay_buy, ucid)-GetDict(user_item_lastThreeDay_buy, ucid),
			]
